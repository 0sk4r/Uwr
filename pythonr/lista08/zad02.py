...
class DB:
    def __init__(self):
        self.database = shelve.open("data")
...
class App:
    def __init__(self):
        ...
        Tkinter.Label(self.frame, text="INFO").grid(
            row=0, column=1, columnspan=2)

        Tkinter.Label(self.frame, text="Title:").grid(row=1, column=1)
        self.str_title = Tkinter.StringVar()
        self.entry_title = Tkinter.Entry(
            self.frame, textvariable=self.str_title).grid(row=1, column=2)

        Tkinter.Label(self.frame, text="Author:").grid(row=2, column=1)
        self.str_author = Tkinter.StringVar()
        self.entry_author = Tkinter.Entry(
            self.frame, textvariable=self.str_author).grid(row=2, column=2)

        Tkinter.Label(self.frame, text="Songs:").grid(row=3, column=1)
        self.str_songs = Tkinter.StringVar()
        self.entry_songs = Tkinter.Entry(
            self.frame, textvariable=self.str_songs).grid(row=3, column=2)

        Tkinter.Label(self.frame, text="Borrowed:").grid(row=4, column=1)
        self.str_borrowed = Tkinter.StringVar()
        self.entry_borrowed = Tkinter.Entry(
            self.frame, textvariable=self.str_borrowed).grid(row=4, column=2)

        self.button_add = Tkinter.Button(
            self.frame, text="Add", command=self.add_cd).grid(row=5, column=2)
        ...
        self.entry_search = Tkinter.Entry(
            self.frame, textvariable=self.str_search).grid(row=6, column=0)
        self.button_search = Tkinter.Button(
            self.frame, text="Search", command=self.search_cd).grid(row=7, column=0)
    ...
    def listbox_select(self, val):
        ...
        Tkinter.Button(self.frame, text="Delete",
                       command=lambda: self.delete_cd(key)).grid(row=5, column=1)
        Tkinter.Button(self.frame, text="Save",
                       command=lambda: self.edit_cd(key)).grid(row=6, column=1)
    ...
    def search_cd(self):
        ...
        if (found):
            ...
            Tkinter.Button(self.frame, text="Delete", command=lambda: self.delete_cd(
                key)).grid(row=5, column=1)
            Tkinter.Button(self.frame, text="Save", command=lambda: self.edit_cd(
                key)).grid(row=6, column=1)

    def add_cd(self):
        self.database.add_cd(self.str_title.get(), self.str_author.get(
        ), self.str_songs.get(), self.str_borrowed.get())
    ...
    def edit_cd(self, old_title):
        self.database.edit_cd(old_title, self.str_title.get(), self.str_author.get(
        ), self.str_songs.get(), self.str_borrowed.get())
        self.listbox.delete(self.listbox.curselection())
        self.listbox.insert(Tkinter.END, self.str_title.get())
