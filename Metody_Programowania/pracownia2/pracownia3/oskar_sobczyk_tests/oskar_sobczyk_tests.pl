...
:- module(oskar_sobczyk_teskts, [tests/3]).
...
tests(unary_op, file('unary_op.hdml'),yes).

tests(more_var_def,input("det test(A,B,C,D,E) = 1"),yes).

tests(more_var_def2,input("def test(A,B,C,D) = 1"),yes).

tests(bitselect, file('bitselect.hdml'), program([def(test,var(no,'A'), bitsel(no,var(no,'A'),num(no,1),num(no,3)))])).

tests(simple_let, file('simple_let.hdml'), program([def(test, var(no, 'A'), let(no, var(no, 'C'),  num(no, 1), op(no, '&', var(no,'A'),var(no,'C'))))])).
